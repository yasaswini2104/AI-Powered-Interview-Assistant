...
export const fetchQuestion = createAsyncThunk(
  ...
  async (_, { getState, rejectWithValue }) => {
    ...
    const { candidateId, role, history } = state.interview;
    
    // For trial mode, we need to pass history as an array of question strings
    const questionHistory = history.map(h => h.question);
    ...
    try {
      const response = await apiClient.post('/interview/question', { 
        ...
        role, 
        difficulty, 
        history: questionHistory // Send history array for trial mode
      });
